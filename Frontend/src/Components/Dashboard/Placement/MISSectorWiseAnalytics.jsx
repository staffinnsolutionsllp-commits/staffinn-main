...
const MISSectorWiseAnalytics = () => {
  ...
  const loadSectors = async () => {
    try {
      setLoading(true);
      const response = await apiService.getMISPlacementSectors();
      setSectors(Array.isArray(response) ? response : []);
    } catch (error) {
      console.error('Error loading MIS sectors:', error);
      setSectors([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadSectorStudents = async (sector) => {
    ...
    try {
      ...
      const response = await apiService.getMISSectorWiseStudents(sector);
      setStudentData(Array.isArray(response) ? response : []);
    } catch (error) {
      console.error('Error loading MIS sector students:', error);
      setStudentData([]);
    } finally {
...
export default MISSectorWiseAnalytics;
