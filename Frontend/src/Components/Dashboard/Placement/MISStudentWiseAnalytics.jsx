...
const MISStudentWiseAnalytics = () => {
  ...
  const loadStudentWiseData = async () => {
    try {
      setLoading(true);
      const response = await apiService.getMISStudentWiseAnalytics();
      setStudentData(Array.isArray(response) ? response : []);
    } catch (error) {
      console.error('Error loading MIS student wise data:', error);
      setStudentData([]);
    } finally {
      setLoading(false);
    }
...
export default MISStudentWiseAnalytics;
