...
const MISCenterWiseAnalytics = () => {
  ...
  const loadTrainingCenters = async () => {
    try {
      setLoading(true);
      const result = await apiService.getMISPlacementTrainingCenters();
      if (result.success) {
      ...
      console.error('Error loading MIS training centers:', error);
      setTrainingCenters([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadCenterStudents = async (centerId) => {
    try {
      ...
      const result = await apiService.getMISCenterWiseStudents(centerId);
      if (result.success) {
      ...
      console.error('Error loading MIS center students:', error);
      setStudentData([]);
    } finally {
...
export default MISCenterWiseAnalytics;
