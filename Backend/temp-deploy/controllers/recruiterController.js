...
const getRecruiterProfile = async (req, res) => {
  try {
    ...
    const completeProfile = {
      ...
      officeLocations: profileData.officeLocations || [],
...
const updateRecruiterProfile = async (req, res) => {
  try {
    ...
    if (updateData.gstNumber && updateData.gstNumber.trim() !== '') {
      const gstRegex = /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/;
      if (!gstRegex.test(updateData.gstNumber.trim())) {
        return res.status(400).json({
          success: false,
          message: 'Invalid GST Number format. GST Number should be 15 characters (e.g., 22AAAAA0000A1Z5)'
        });
      }
    }
    
    // Validate PAN Number if provided
    if (updateData.panNumber && updateData.panNumber.trim() !== '') {
      const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
      if (!panRegex.test(updateData.panNumber.trim().toUpperCase())) {
        return res.status(400).json({
          success: false,
          message: 'Invalid PAN Number format. PAN should be 10 characters (e.g., ABCDE1234F)'
        });
      }
      // Ensure PAN is stored in uppercase
      updateData.panNumber = updateData.panNumber.trim().toUpperCase();
    }
    ...
    const profileData = {
      ...
      // Ensure officeLocations are preserved
      officeLocations: updateData.officeLocations !== undefined ? updateData.officeLocations : (existingProfile.officeLocations || []),
    ...
    // DEBUG: Log officeLocations being saved
    console.log('🏢 SAVING officeLocations to DynamoDB:', {
      table: RECRUITER_PROFILES_TABLE,
      recruiterId: userId,
      officeLocationsCount: (profileData.officeLocations || []).length,
      officeLocations: JSON.stringify(profileData.officeLocations || [])
    });
    ...
    // Return the data we just saved directly (avoids DynamoDB eventual consistency issues)
    // Merge user registration data with the profile data we just wrote
    const responseData = {
      ...currentUser,
      ...profileData
    };
    ...
    console.log('✅ RETURNING saved data in response:', {
      recruiterId: userId,
      officeLocationsCount: (profileData.officeLocations || []).length,
      officeLocations: JSON.stringify(profileData.officeLocations || [])
    });
  ...
  try {
    ...
    const formattedRecruiters = recruiters.map(recruiter => {
      ...
      return {
        ...
        officeLocations: useProfile.officeLocations || [],
        // Social links
        linkedin: useProfile.linkedin || null,
        twitter: useProfile.twitter || null,
        instagram: useProfile.instagram || null,
        facebook: useProfile.facebook || null,
        youtube: useProfile.youtube || null,
        github: useProfile.github || null,
        // Awards & Achievements
        awards: useProfile.awards || [],
...
const getRecruiterByIdPublic = async (req, res) => {
  try {
    ...
    console.log('Public recruiter profile data:', {
      ...
      officeLocationsCount: profile.officeLocations ? profile.officeLocations.length : 0,
      profilePhoto: profile.profilePhoto,
    ...
    const formattedRecruiter = {
      // Spread ALL profile fields first so nothing is missed
      ...useProfile,
      // Then override with explicitly formatted fields
      id: recruiter.userId,
...
/**
 * Upload office location image to S3 (no DB update, just returns URL)
 * @route POST /api/recruiter/upload-location-image
 */
const uploadLocationImage = async (req, res) => {
  try {
    ...
    const s3Service = require('../services/s3Service');
    const fileExtension = req.file.originalname.split('.').pop();
    const s3Key = `recruiter-profiles/${req.user.userId}/office-locations/loc-img-${Date.now()}.${fileExtension}`;
    ...
    console.log('✅ Location image uploaded to S3:', uploadResult.Location);
...
const updateCandidateStatus = async (req, res) => {
  try {
    ...
    // ── When a candidate is hired, also write a hiring record ────────────────
    // This ensures "My Hirings → Hiring History" shows staff/seeker hires.
    // Only for 'Hired' — rejected candidates do NOT appear in hiring history.
    if (status === 'Hired') {
      try {
        ...
        // Get staff (candidate) details
        const staffProfile = await staffModel.getStaffProfile(staffId);

        // Find the specific application to get jobId / jobTitle
        const application = (staffProfile?.applications || []).find(
          a => a.applicationId === applicationId
        );

        const hiringData = {
          applicantType:   'staff',       // ← marks this as a staff/seeker hire, NOT institute
          studentID:       staffId,
          recruiterID:     req.user.userId,
          recruiterName:   recruiterUser?.name || 'Recruiter',
          recruiterCompany: req.user.companyName || recruiterUser?.companyName || '',
          jobID:           application?.jobId   || applicationId,
          jobTitle:        application?.jobTitle || 'Position',
          status:          'Hired',
          studentSnapshot: staffProfile ? {
        ...
        await hiringModel.createHiringRecord(hiringData);
        console.log(`✅ Hiring record created for staff candidate ${staffId}`);
      } catch (hrErr) {
        // Non-fatal — status update already succeeded
        console.error('Failed to write hiring record (non-fatal):', hrErr.message);
      }
    }
