  ...
  static async sendMessage(req, res) {
    try {
      ...
      try {
        ...
        if (io) {
          // Notify receiver so their sidebar/chat window updates instantly
          io.to(`user-${receiverId}`).emit('new-message', {
            message: newMessage,
            timestamp: new Date().toISOString()
          });
          // Also notify sender's other tabs/sessions
        ...
        console.error('WebSocket emit error (new-message):', wsErr);
      }
  ...
  static async sendFileMessage(req, res) {
    try {
      ...
      try {
        ...
        console.error('WebSocket emit error (new-message file):', wsErr);
      }
