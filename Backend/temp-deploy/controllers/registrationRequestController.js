...
const approveRequest = async (req, res) => {
  try {
    ...
    // IDEMPOTENCY CHECK: Prevent re-processing already approved requests
    if (request.status === 'approved') {
      console.log('⚠️ Request already approved, skipping...');
    ...
    if (existingUser) {
      ...
      const userData = {
        email: request.email,
        password: password, // Pass plaintext - createUser will hash it
        role: request.type,
    ...
    const emailSent = await sendApprovalEmail(request.email, request.name, password, request.type);
    
    if (!emailSent) {
      console.error('❌ Failed to send approval email');
      return res.status(500).json({ 
        success: false, 
        message: 'User account created but failed to send email. Please contact support.' 
      });
    }
    
    console.log('✅ Approval email sent successfully');
    
    // Update request status ONLY after email is sent successfully
