...
const OTP_CONFIG = {
  ...
  RATE_LIMIT: {
    MAX_REQUESTS: 5, // Max 5 OTP requests per email
...
const generateResetToken = async (email) => {
  try {
    ...
    // Store reset token
    const tokenKey = 'reset_' + normalizedEmail;
    otpStorage.set(tokenKey, {
      token: resetToken,
      createdAt: Date.now(),
      expiryTime,
      used: false
    ...
    console.log('Reset token generated for email: ' + normalizedEmail);
    return resetToken;
...
const verifyResetToken = async (email, resetToken) => {
  try {
    ...
    if (!otpStorage.has(tokenKey)) {
      console.error('No reset token found for email: ' + normalizedEmail);
      return false;
    }
    
    const tokenData = otpStorage.get(tokenKey);
    
    // Check if expired
    if (Date.now() > tokenData.expiryTime) {
      console.error('Reset token expired for email: ' + normalizedEmail);
    ...
    if (tokenData.used) {
      console.error('Reset token already used for email: ' + normalizedEmail);
      return false;
    }
    
    // Verify token
    if (tokenData.token !== resetToken) {
      console.error('Invalid reset token for email: ' + normalizedEmail);
      return false;
    }
    
    console.log('Reset token verified for email: ' + normalizedEmail);
    return true;
...
const clearResetToken = async (email) => {
  try {
    ...
    if (otpStorage.has(tokenKey)) {
      otpStorage.delete(tokenKey);
      console.log('Reset token cleared for email: ' + normalizedEmail);
    }
