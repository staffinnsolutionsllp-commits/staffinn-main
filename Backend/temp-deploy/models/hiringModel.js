...
/**
 * Create a new hiring record (with deduplication)
 * If a record for the same student + job + recruiter already exists, update it.
 * This prevents duplicate entries in Hiring History.
 * @param {object} hiringData - Hiring record data
 * @returns {Promise<object>} - Created or updated hiring record
 */
const createHiringRecord = async (hiringData) => {
  try {
    const timestamp = new Date().toISOString();

    // Check for an existing record with same student + job + recruiter
    const studentId  = hiringData.studentID || hiringData.staffId;
    const jobId      = hiringData.jobID;
    const recruiterId = hiringData.recruiterID || hiringData.seekerId;

    if (studentId && jobId && recruiterId) {
      const existing = await dynamoService.scanItems(HIRING_TABLE, {
        FilterExpression: '(studentID = :sid OR staffId = :sid) AND jobID = :jid AND (recruiterID = :rid OR seekerId = :rid)',
      ...
      if (existing && existing.length > 0) {
        // Update the most recent existing record instead of creating a new one
        const existingRecord = existing.sort((a, b) =>
          new Date(b.timestamp || b.createdAt || 0) - new Date(a.timestamp || a.createdAt || 0)
        )[0];

        const updatedRecord = {
          ...existingRecord,
          status:          hiringData.status || existingRecord.status,
          jobTitle:        hiringData.jobTitle || existingRecord.jobTitle,
          studentSnapshot: hiringData.studentSnapshot || existingRecord.studentSnapshot,
          timestamp,
          updatedAt: timestamp
        };

        await dynamoService.putItem(HIRING_TABLE, updatedRecord);
        console.log(`✅ Updated existing hiring record ${existingRecord.hiringRecordID} (no duplicate created)`);
    ...
    const hiringRecord = {
      hiringRecordID,
      applicantType: hiringData.applicantType,
      studentID: studentId,
...
/**
 * Get hiring records for a recruiter — STAFF CANDIDATES ONLY.
 * Filters to applicantType = 'staff' so that institute placement records
 * (applicantType = 'institute') never appear in My Hirings → Hiring History.
...
const getRecruiterHiringHistory = async (recruiterId) => {
  try {
    const scanParams = {
      FilterExpression: '(recruiterID = :recruiterId OR seekerId = :recruiterId) AND (applicantType = :staff OR attribute_not_exists(applicantType))',
      ExpressionAttributeValues: {
        ':recruiterId': recruiterId,
        ':staff': 'staff'
      }
