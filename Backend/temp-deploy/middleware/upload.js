...
const fileFilter = (req, file, cb) => {
  try {
    ...
    switch (file.fieldname) {
      ...
      case 'syllabus':
        // Allow PDF and DOC/DOCX files for course syllabus
        const allowedSyllabusTypes = [
          'application/pdf',
          'application/msword',
          'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
        ];
        if (allowedSyllabusTypes.includes(file.mimetype)) {
          cb(null, true);
        } else {
          cb(new Error('Syllabus must be a PDF or DOC/DOCX file'), false);
        }
        break;
...
const uploadConfig = multer({
  storage: storage,
  limits: {
    fileSize: 20 * 1024 * 1024, // 20MB limit (increased for syllabus docs)
